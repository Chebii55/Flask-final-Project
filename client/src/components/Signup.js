...
function SignupForm() {
  const [userOrStaff, setUserOrStaff] = useState('user'); // Default to user signup

  const handleSignup = (e) => {
    e.preventDefault();
    // Add logic to handle signup based on userOrStaff state
    console.log('Signing up as:', userOrStaff);
  };
      ...
      <div className="bg-white lg:w-5/12 md:w-6/12 w-10/12 shadow-3xl">
        ...
        <form className="p-12 md:p-24" onSubmit={handleSignup}>
          ...
          <div className="flex items-center text-lg mb-6 md:mb-8">
            ...
            <input type="text" id="name" className="bg-gray-200 pl-12 py-2 md:py-4 focus:outline-none w-full" placeholder="Name" />
          </div>
          <div className="flex items-center text-lg mb-6 md:mb-8">
            <svg className="absolute ml-3" viewBox="0 0 24 24" width="24">
              <path d="m18.75 9h-.75v-3c0-3.309-2.691-6-6-6s-6 2.691-6 6v3h-.75c-1.24 0-2.25 1.009-2.25 2.25v10.5c0 1.241 1.01 2.25 2.25 2.25h13.5c1.24 0 2.25-1.009 2.25-2.25v-10.5c0-1.241-1.01-2.25-2.25-2.25zm-10.75-3c0-2.206 1.794-4 4-4s4 1.794 4 4v3h-8zm5 10.722v2.278c0 .552-.447 1-1 1s-1-.448-1-1v-2.278c-.595-.347-1-.985-1-1.722 0-1.103.897-2 2-2s2 .897 2 2c0 .737-.405 1.375-1 1.722z"/>
            </svg>
            <input type="text" id="phone" className="bg-gray-200 pl-12 py-2 md:py-4 focus:outline-none w-full" placeholder="Phone" />
          </div>
          <div className="flex items-center text-lg mb-6 md:mb-8">
            <svg className="absolute ml-3" viewBox="0 0 24 24" width="24">
              <path d="m18.75 9h-.75v-3c0-3.309-2.691-6-6-6s-6 2.691-6 6v3h-.75c-1.24 0-2.25 1.009-2.25 2.25v10.5c0 1.241 1.01 2.25 2.25 2.25h13.5c1.24 0 2.25-1.009 2.25-2.25v-10.5c0-1.241-1.01-2.25-2.25-2.25zm-10.75-3c0-2.206 1.794-4 4-4s4 1.794 4 4v3h-8zm5 10.722v2.278c0 .552-.447 1-1 1s-1-.448-1-1v-2.278c-.595-.347-1-.985-1-1.722 0-1.103.897-2 2-2s2 .897 2 2c0 .737-.405 1.375-1 1.722z"/>
          ...
          {userOrStaff === 'staff' && (
            <div className="flex items-center text-lg mb-6 md:mb-8">
              ...
              <input type="text" id="staffId" className="bg-gray-200 pl-12 py-2 md:py-4 focus:outline-none w-full" placeholder="Staff ID" />
            </div>
          )}
          <div className="flex items-center text-lg mb-6 md:mb-8">
            <svg className="absolute ml-3" viewBox="0 0 24 24" width="24">
          ...
          <button className="bg-gradient-to-b from-blue-700 to-blue-900 font-medium p-2 md:p-4 text-white uppercase w-full">Sign Up</button>
        </form>
      </div>
    </div>
  );
